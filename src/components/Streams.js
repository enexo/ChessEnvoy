...
function LiveStreams() {
    ...
    const [hnLive, setHNLive] = useState('unknown');
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            // Hikaru Nakamura
            const hn = await api.get('https://api.twitch.tv/helix/search/channels?query=GMHikaru');
            let hnResult = hn.data.data[0].is_live;
            const hnLive = "Live on Twitch!";
            // If Live, then post live; needs improvement because it prints "Unknown" if statement is False
            if (hnResult) {
                setHNLive(hnLive);
            }
